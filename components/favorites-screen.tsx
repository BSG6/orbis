...
import { getAllFavorites, getProblemById, type FavoriteRecord } from "@/lib/db"
...
export function FavoritesScreen({ isFocusMode }: FavoritesScreenProps) {
  ...
  const [problems, setProblems] = useState<FavoriteProblem[]>([])
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        ...
        const out: FavoriteProblem[] = []
        for (const fav of favs as FavoriteRecord[]) {
          const p = await getProblemById(fav.problemId)
          if (!p) continue
          out.push({
            id: p.id,
            title: p.title,
