...
import { getProblemById, type ProblemRecord } from "@/lib/db"
...
export function ReviewScreen({ isFocusMode }: ReviewScreenProps) {
  ...
  const { getDueProblemsForToday, snoozeProblemById } = useScheduling()

  // Derived: due schedules from hook
  const dueSchedules = useMemo(() => getDueProblemsForToday(), [getDueProblemsForToday])

  // Local display records derived from schedules + problems store
  const [displayProblems, setDisplayProblems] = useState<ReviewProblem[]>([])
  ...
  useEffect(() => {
    const load = async () => {
      setIsLoading(true)
      try {
        const items: ReviewProblem[] = []
        for (const sched of dueSchedules) {
          const p = (await getProblemById(sched.problemId)) as ProblemRecord | undefined
          if (!p) continue
          // Map minimal fields for UI (difficulty/pattern are placeholders for now)
          items.push({
            id: p.id,
            title: p.title,
            difficulty: "Medium",
            pattern: (p.tags && p.tags[0]) || "General",
            lastAttempt: "—",
            dueDate: "Today",
            confidence: Math.max(1, Math.min(5, sched.lastRating || 3)),
            attempts: sched.ratingHistory?.length || 0,
          })
        }
        setDisplayProblems(items)
      } finally {
        setIsLoading(false)
      }
    }
  ...
  const handleSnooze = async (problemId: string) => {
    setSnoozedProblems((prev) => new Set([...Array.from(prev), problemId]))
    await snoozeProblemById(problemId)
  }
  ...
  const activeProblem = displayProblems.filter((p) => !snoozedProblems.has(p.id))
